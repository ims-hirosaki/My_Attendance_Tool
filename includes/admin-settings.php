 ...
 * 既存の備考（note）が空で上書きされて消滅しないよう、サーバー側で既存データを保護・マージするロジックを実装。
 */
...
function mat_attendance_update_handler() {
    ...
    $note_input    = sanitize_textarea_field( $_POST['note'] ?? '' ); 
    $today         = current_time( 'Y-m-d' );
    ...
    // 【重要】備考先入れ上書き対策：既存の備考データと綺麗に結合・保持
...
function mat_save_note_handler() {
    ...
    if ( $row ) {
        ...
        if ( $updated === false ) wp_send_json_error( '備考の保存に失敗しました。' );
    } else {
        ...
        if ( ! $inserted ) wp_send_json_error( '備考の保存に失敗しました。' );
    }
